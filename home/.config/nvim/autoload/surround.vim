...
function! surround#Add(key) range
	...
	call setpos('.',l:end  ) | execute "norm! a" .. s:matchpairs_dict[a:key][1] .. "\e"
	call setpos('.',l:start) | execute "norm! i" .. s:matchpairs_dict[a:key][0] .. "\e"
endfunction
...
function! surround#Delete(symbol='')
	if !empty(a:symbol)
		execute "norm! va" .. a:symbol .. "\e"
	endif 
	if empty(a:symbol) && !<SID>in_matchpairs()
		return
	endif 
	let [ l:l1, l:c1 ] = [ line("."), col(".") ] | norm! %
	let [ l:l2, l:c2 ] = [ line("."), col(".") ] | norm! %
	if l:l1 == l:l2 && l:c1 > l:c2
